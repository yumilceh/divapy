...
% update the models (no historical data is used).
%
...
%% Update of the GMM parameters (generative method).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Initial recursive learning rate
alpha = .5;
%Number of samples to use for the generative update process
nbTrainingSamples = 5;
%Recursive update
for i=2:nbSamples
  [M(i).Priors, M(i).Mu, M(i).Sigma, alpha] = EM_update_generativeMethod(...
    Data(:,(i-1)*nbData+1:i*nbData), M(i-1).Priors, M(i-1).Mu, M(i-1).Sigma,...
    nbData, nbTrainingSamples, alpha);
end

%% Plot of the incremental update results.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
figure('position',[10,50,1000,600],'name','GMM-incremental-demo2');
for i=1:nbSamples
  subplot(2,3,i); hold on;
  plotGMM(M(i).Mu, M(i).Sigma, [.8 0 0], 1);
  plot(Data(1,1:i*nbData),Data(2,1:i*nbData), 'x', 'markerSize', 4, 'color', [.9 .6 .6]);
