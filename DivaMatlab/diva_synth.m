...
%      fprintf('time %d: sum_synth_filt %d + %d j\n', time, sum(real(synth.filt)), sum(imag(synth.filt)))
...
%         fprintf('time %d: sum_v %d \n', time, sum(v))
        ...
        idx_sample = ceil(numel(synth.sample)*(1:synth.samplesperperiod)'/synth.samplesperperiod);
        v=(1-w).*synth.sample(idx_sample)+w.*vnew;
        synth.sample=vnew;
